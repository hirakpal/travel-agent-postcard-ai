...
def get_users_from_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (username TEXT PRIMARY KEY, name TEXT, password TEXT)''')
    cursor.execute("SELECT count(*) FROM users WHERE username='admin'")
    if cursor.fetchone()[0] == 0:
        hashed = stauth.Hasher().hash('admin123')
        cursor.execute("INSERT INTO users VALUES (?, ?, ?)", ('admin', 'Administrator', hashed))
        conn.commit()
...
authenticator = stauth.Authenticate({"usernames": get_users_from_db()}, 'postcard_cookie', 'secret_key', 30)

...
if st.session_state.page == 'login':
    ...
    if st.button("Register"):
        try:
            conn = sqlite3.connect('users.db')
            conn.execute("INSERT INTO users VALUES (?, ?, ?, ?)", (new_user, new_name, stauth.Hasher().hash(new_pass), new_email))
            conn.commit(); conn.close()
            st.success("Account created!")
        except: st.error("Registration failed (Username exists).")
    ...
    if st.button("Send Recovery Token"):
        ...
        if user:
            ...
            st.info(f"Recovery token generated: {token}")
    ...
    if st.button("Update Password"):
        if token_in in st.session_state.recovery_tokens:
            ...
            conn.execute("UPDATE users SET password=? WHERE username=?", 
                         (stauth.Hasher().hash(new_p), st.session_state.recovery_tokens[token_in]))
            conn.commit(); conn.close()

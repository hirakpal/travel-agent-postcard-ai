...
class AgentState(TypedDict):
    itinerary: dict  # Stored as dict for state compatibility
...
def plan_itinerary(destination: str, feedback_history: List[str] = None):
    ...
    prompt = f"Create a 2-day travel plan for {destination}. {context}. Return names, latitude, and longitude for each node."
...
def planning_node(state: AgentState):
    print(">>> Planning Agent: Verifying state sequence and generating plan...")
...
def reprice_node(state: AgentState):
    print(">>> ALERT: Pricing is stale. Re-fetching live data...")
...
def feedback_node(state: AgentState):
    print(">>> Feedback Agent: Updating user profile...")
    return {"feedback": state.get('feedback', []) + ["User preference updated"]}

# --- 4. Orchestration & Graph Construction ---
...
# Compile the app for import by main.py
app = builder.compile()
